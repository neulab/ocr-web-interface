...
function NewlineText(props) {
  ...
  const newText = text.split('\n').map((str,index) => <p className='mb-0' key={index}>{str}</p>);
  return newText;
}
...
function DisplayImages(props) {
  ...
  if (props.imgUploads) {
    downloadButton = <Row className="justify-content-center">
          <Col xs="auto">
            <Button variant="secondary" onClick={() => handleDownload(props.imgUploads)}>Download OCR Outputs</Button>
          </Col>
        </Row>;
        ...
        <Tab eventKey="ocr" title="Off-the-shelf OCR">
          <Container>
            <Row className="justify-content-center">
              <Col xs="auto">
                <UploadForm handleSubmit={handleSubmit} handleFileSelect={handleFileSelect}/>
              </Col>
            </Row>
